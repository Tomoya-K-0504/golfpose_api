...
def read_cap(img, scale_factor=1.0, output_stride=16):    
    ...
    height = int(frame.shape[0] * 1.0)
    width = int(frame.shape[1] * 1.0)
    upscaled = cv2.copyMakeBorder(frame, height, height, width, width, cv2.BORDER_CONSTANT, value=(0, 0, 0))
    img = cv2.resize(upscaled, (frame.shape[1], frame.shape[0]))
